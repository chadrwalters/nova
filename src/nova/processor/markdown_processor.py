...
class MarkdownProcessor:
    ...
    def _extract_attachments(self, content: str, source_file: str) -> List[Dict[str, Any]]:
        ...
        references = self.reference_manager.extract_references(content, Path(source_file))
        ...
        for reference in references:
            if reference.ref_type == 'ATTACH':
                ...
                attachment = {
                    'path': str(reference.target_file) if reference.target_file else reference.ref_id,
                    'text': reference.ref_id,
                    'context': reference.context,
                    'section': 'attachments',  # Default to attachments section
                    'source_file': source_file,
                    'date': self._extract_date(source_file),
                    'type': reference.ref_id.split(':')[0] if ':' in reference.ref_id else 'OTHER',
    ...
    def _build_attachments_markdown(self, attachments: List[Dict[str, Any]]) -> str:
        ...
        for date in sorted(by_date.keys()):
            ...
            for file_type in sorted(by_date[date].keys()):
                ...
                for attachment in sorted(by_date[date][file_type], key=lambda x: x['text']):
