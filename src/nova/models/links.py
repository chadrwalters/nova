...
class LinkType(str, Enum):
    ...
    ATTACHMENT_TO_SUMMARY = "attachment_to_summary"  # Attachment points to summary section
    ATTACHMENT_TO_NOTES = "attachment_to_notes"  # Attachment points to raw notes section
...
class LinkContext(BaseModel):
    ...
    source_context: Optional[str] = Field(None, description="Surrounding text in source file")
    ...
    target_context: Optional[str] = Field(None, description="Surrounding text in target file")
    
    # Link information
    link_type: LinkType = Field(default=LinkType.OUTGOING, description="Type of link relationship")
    ...
    created_at: datetime = Field(default_factory=datetime.now, description="When the link was created")
...
class LinkMap(BaseModel):
    ...
    outgoing_links: Dict[str, List[LinkContext]] = Field(
        ...
        description="Links from this document to others"
    )
    
    # Links by target file
    incoming_links: Dict[str, List[LinkContext]] = Field(
        default_factory=dict,
        description="Links from other documents to this one"
    )
    ...
    def _has_circular_reference(self, current: str, path: List[str], visited: set) -> bool:
