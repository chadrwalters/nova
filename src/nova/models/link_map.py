...
from .links import LinkType, LinkContext
...
class NavigationPath(BaseModel):
    ...
    last_validated: datetime = Field(default_factory=datetime.now, description="When the path was last validated")
...
class LinkRelationshipMap(BaseModel):
    ...
    navigation_paths: Dict[Tuple[str, str], List[NavigationPath]] = Field(
        ...
        description="Map of file pairs to their navigation paths"
    )
    ...
    def find_navigation_paths(
        ...
        def dfs(current: str, path: List[str], depth: int) -> None:
            ...
            if current == target:
                ...
                nodes = [
                    NavigationNode(file_path=file, section_id="")
                    for file in path
                ]
                path_type = (
                    NavigationPathType.DIRECT if len(path) == 2
    ...
    def _repair_fuzzy_match(
        ...
        if similarity >= 0.8:
            ...
            result.repair_notes = f"Best match {best_match} not confident enough ({similarity:.2f})"
        ...
        for target in available_files:
            ...
            if paths:
                ...
                if shortest.total_links <= 2:  # Only use short paths
                    ...
                    result.repair_notes = f"Found alternative path through {shortest.total_links} links"
                    return result
    ...
    def get_repair_suggestions(
        ...
        if fuzzy_result.repaired_link:
            suggestions.append((
                fuzzy_result.repaired_link.target_file,
        ...
        if path_result.repaired_link:
            suggestions.append((
                path_result.repaired_link.target_file,
                path_result.confidence
            ))
        ...
        if alt_result.repaired_link:
            suggestions.append((
                alt_result.repaired_link.target_file,
