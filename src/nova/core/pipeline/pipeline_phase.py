...
class PipelinePhase:
    ...
    def __init__(self, phase_config: Dict[str, Any], pipeline_config: Optional[Dict[str, Any]] = None):
        ...
        """
        self.name = phase_config.get('name', self.__class__.__name__)
        self.description = phase_config.get('description', '')
        self.options = phase_config.get('options', {})
        self.components = phase_config.get('components', {})
        self.output_dir = phase_config.get('output_dir')
        self.dependencies = phase_config.get('dependencies', [])
    ...
    async def process(self, input_files: List[Path]) -> ProcessingResult:
        ...
        try:
            ...
            error_msg = f"Error in phase {self.name}: {str(e)}"
