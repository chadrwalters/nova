...
@dataclass
class TimingManager:
    """Manages timing operations and thresholds."""
    warn_threshold: float = 5.0  # seconds
    ...
    def timer(self, name: str):
        ...
        try:
            ...
            if duration > self.error_threshold:
                raise PipelineError(f"Operation {name} took {duration:.2f}s (error threshold: {self.error_threshold}s)")
            elif duration > self.warn_threshold:
                print(f"Warning: Operation {name} took {duration:.2f}s (warning threshold: {self.warn_threshold}s)")
    
    def get_timing(self, name: str) -> List[float]:
    ...
    def get_average_timing(self, name: str) -> Optional[float]:
        ...
        Returns:
            Average timing or None if no data
        """
        timings = self.get_timing(name)
...
__all__ = ['TimingManager', 'TimingMetrics', 'TimingEnhancer'] 