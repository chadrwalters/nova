...
class NovaPipeline:
    ...
    async def process_directory(
        self, directory: Union[str, Path], phases: List[str] = None
    ) -> bool:
        ...
        try:
            ...
            with self.progress:
                ...
                for file_path in input_files:
                    try:
                        metadata = await self.phases["parse"].process_file(
                            file_path, parse_output_dir
                        )
                        if metadata:
                ...
                for file_path in parsed_files:
                    try:
                        metadata = await self.phases["disassemble"].process_file(
                            file_path, disassemble_output_dir
                        )
                        if metadata:
                ...
                for file_path in summary_files:
                    try:
                        metadata = await self.phases["split"].process_file(
                            file_path, split_output_dir
                        )
                        if metadata:
                ...
                for file_path in split_files:
                    try:
                        metadata = await self.phases["finalize"].process_file(
                            file_path, finalize_output_dir
                        )
                        if metadata:
            ...
            for phase_name in ["parse", "disassemble", "split", "finalize"]:
                ...
                if phase_state["failed_files"]:
                    ...
                    for file_path in phase_state["failed_files"]:
                        ...
                        if (
                            phase_name in self.error_messages
                            and file_path in self.error_messages[phase_name]
                        ):
                            console.print(
                                f"  Error: {self.error_messages[phase_name][file_path]}",
                                style="red",
                            )
                        console.print()
