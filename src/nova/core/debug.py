...
class DebugManager:
    ...
    def take_state_snapshot(self, state: Dict, phase: str) -> None:
        ...
        if self.config.dump_state and self.config.dump_dir:
            dump_file = (
                ...
                / f"state_{phase}_{len(self.state_snapshots)}.json"
            )
            ...
            if set_name in phase_state:
                ...
                for file_path in file_set:
                    if not Path(file_path).exists():
                        errors.append(
                            f"File in {phase}.{set_name} does not exist: {file_path}"
                        )
    ...
    def print_debug_summary(self) -> None:
        ...
        table.add_row(
            "State Logging", "Enabled" if self.config.state_logging else "Disabled"
        )
        table.add_row(
            "Extra Validation",
            "Enabled" if self.config.extra_validation else "Disabled",
        )
        table.add_row(
            "Performance Tracking",
            "Enabled" if self.config.performance_tracking else "Disabled",
        )
        table.add_row(
            "Memory Tracking", "Enabled" if self.config.memory_tracking else "Disabled"
        )
    ...
    def handle_error(
        self, error: Exception, phase: str, file_path: Optional[Path] = None
    ) -> bool:
