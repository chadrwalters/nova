"""Content handler for consolidating markdown files."""
...
class ContentHandler(BaseHandler):
    ...
    async def process(self, file_path: Path, context: Dict[str, Any]) -> HandlerResult:
        ...
        try:
            ...
            # Consolidate content
            consolidated_content = self._consolidate_content(content)

            # Write consolidated content
            output_dir = Path(context['output_dir'])
            output_file = output_dir / f"{file_path.stem}_consolidated.md"
    ...
    def _consolidate_content(self, content: str) -> str:
        """Consolidate content.
        ...
        sections = []
        current_section = []
        in_consolidate_block = False

        # Process content line by line
        for line in content.split('\n'):
            # Check for consolidate block markers
            if line.startswith('--==CONSOLIDATE_BLOCK:'):
                in_consolidate_block = True
                if current_section:
                    sections.append('\n'.join(current_section))
                current_section = [line]
                continue
            elif line.startswith('--==CONSOLIDATE_BLOCK_END==--'):
                in_consolidate_block = False
