...
class MarkdownConsolidateProcessor(BaseProcessor):
    ...
    def __init__(self, processor_config: ProcessorConfig, pipeline_config: PipelineConfig):
        ...
        # Set up directories
        self.input_dir = Path(os.environ.get('NOVA_PHASE_MARKDOWN_PARSE'))
        self.output_dir = Path(os.environ.get('NOVA_PHASE_MARKDOWN_CONSOLIDATE'))
    ...
    async def process(self) -> bool:
        ...
        try:
            ...
            logger.error(f"Error in markdown consolidate processor: {str(e)}")
            return False
