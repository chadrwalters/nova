...
class NavigationRenderer:
    ...
    def render_tooltip(
        self, link: LinkContext, navigation_path: Optional[NavigationPath] = None
    ) -> str:
...
def inject_navigation_elements(content: str, header: NavigationHeader) -> str:
    ...
    if '<div class="nova-nav-header">' in main_content:
        nav_end = main_content.find(
            "</div>", main_content.find('<div class="nova-nav-header">')
        )
        if nav_end != -1:
...
def add_tooltips_to_links(
    content: str, links: List[LinkContext], paths: Dict[str, NavigationPath] = None
) -> str:
    ...
    for link in links:
        ...
        replacement = (
            rf'<span class="nova-link-wrapper" data-tooltip="{tooltip}">\g<0></span>'
        )
