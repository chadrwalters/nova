...
def test_generate_anchor_id(converter_factory):
    ...
    # Special characters
    assert converter_factory.generate_anchor_id("Test & Title!") == "test-and-title"
    assert converter_factory.generate_anchor_id("Section 1.2.3") == "section-1-2-3"
    assert converter_factory.generate_anchor_id("Special $#@% Characters") == "special-characters"
    assert converter_factory.generate_anchor_id("Multiple   Spaces") == "multiple-spaces"
    assert converter_factory.generate_anchor_id("Mixed-Case_Text") == "mixed-case-text"
...
def test_anchor_id_special_characters(converter_factory):
    ...
    cases = [
        ...
        ("Test and Title", "test-title"),  # Tests "and" replacement
        ("Test Title", "test-title"),  # Base case
        ("Test Title", "test-title-1")  # Duplicate handling
    ]
...
"""
    ...
    assert f"![Image 1]({tmp_path}/image1.jpg)" in processed
    assert f"![Image 2]({tmp_path}/image2.png)" in processed
...
def test_normalize_path_error(converter_factory, tmp_path, caplog):
    ...
    with pytest.raises(ProcessingError):
        converter_factory.normalize_path(invalid_path)
...
"""
    ...
    assert "prev.md" in processed
    assert "next.md" in processed
    assert "← [Previous](prev.md)" in processed
    assert "→ [Next](next.md)" in processed
...
def test_validate_navigation_links(converter_factory, tmp_path):
    ...
    converter_factory._nav_links[str(tmp_path / "file1.md")] = {
        'next': str(tmp_path / "file2.md"),
        'parent': str(tmp_path / "parent.md")
    }
    converter_factory._nav_links[str(tmp_path / "file2.md")] = {
        'prev': str(tmp_path / "file1.md"),
        'parent': str(tmp_path / "parent.md")
    }
...
def test_validate_navigation_links_broken_chain(converter_factory, tmp_path):
    ...
    converter_factory._nav_links[str(tmp_path / "file2.md")] = {
        'prev': str(tmp_path / "wrong.md")  # Wrong prev link
    }

    errors = converter_factory.validate_navigation_links()
    assert len(errors) == 1
    assert "Failed to resolve prev link" in errors[0]
...
def test_validate_navigation_links_missing_parent(converter_factory, tmp_path):
    ...
    converter_factory._nav_links[str(tmp_path / "file1.md")] = {
        'parent': str(tmp_path / "missing_parent.md")
    }

    errors = converter_factory.validate_navigation_links()
    assert len(errors) == 1
    assert "Failed to resolve parent link" in errors[0]
