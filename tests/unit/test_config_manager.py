...
class TestConfigManager:
    ...
    def test_config_validation(self, mock_fs, monkeypatch):
        ...
        with pytest.raises(
            ValueError, match="Cache configuration must be a dictionary"
        ):
            ConfigManager(config_path, create_dirs=False)
        ...
        with pytest.raises(
            ValueError, match="Cache configuration missing required field: dir"
        ):
            ConfigManager(config_path, create_dirs=False)
    ...
    def test_path_resolution(self, mock_fs):
        ...
        config_data = {
            ...
            "cache": {"dir": "${TEST_CACHE_DIR}", "enabled": True, "ttl": 3600},
        }
